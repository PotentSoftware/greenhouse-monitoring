	...
	while (true) {
		switch (task_state) {
			...
			case STATE_RUN:   // Initialized and running
				// Wait for vsync to be asserted with small delay to reduce CPU usage
				while (gpio_get_level((gpio_num_t) lep_vsync_pin) == 0) {
					vTaskDelay(pdMS_TO_TICKS(1));  // Back to original 1ms delay
				}
				...
				if (vospi_transfer_segment(vsyncDetectedUsec)) {
					...
					// Back to original delay after successful capture
					...
					if (++vsync_count == 36) {  // Back to original
						vsync_count = 0;
						...
						// Back to moderate delay that was working
