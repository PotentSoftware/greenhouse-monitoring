...
bool vospi_transfer_segment(uint64_t vsyncDetectedUsec)
{
	...
	// Debug transfer attempts periodically
	if (transfer_count <= 5 || (transfer_count % 100) == 0) {
		ESP_LOGI(TAG, "Transfer attempt #%d (success: %d/%d)", transfer_count, success_count, transfer_count);
	}
	...
	while (!done) {
		if (transfer_packet(&line, &segment)) {
			...
			if (transfer_count <= 5) {
				ESP_LOGI(TAG, "Transfer timeout after %lld usec", esp_timer_get_time() - vsyncDetectedUsec);
			}
      		done = true;
    	}
	}
	...
	if (success) {
		success_count++;
		if (transfer_count <= 5) {
			ESP_LOGI(TAG, "Transfer #%d SUCCESS - got complete frame", transfer_count);
		}
	} else if (transfer_count <= 5) {
		ESP_LOGI(TAG, "Transfer #%d FAILED - no complete frame", transfer_count);
	}
...
static bool transfer_packet(uint8_t* line, uint8_t* seg)
{
	...
	*seg = 0;
	...
	if (ret != ESP_OK) {
		ESP_LOGE(TAG, "SPI transmit failed: %s (%d)", esp_err_to_name(ret), ret);
		return false;
	}
	
	// Debug packet data periodically
	if (debug_count < 10) {
		ESP_LOGI(TAG, "SPI packet[0-3]: 0x%02X 0x%02X 0x%02X 0x%02X", 
				lepPacketP[0], lepPacketP[1], lepPacketP[2], lepPacketP[3]);
		debug_count++;
	}
	...
	if ((*lepPacketP & 0x0F) == 0x0F) {
		valid = false;
		if (debug_count < 10) {
			ESP_LOGI(TAG, "Sync packet detected (0x%02X)", *lepPacketP);
		}
	} else {
		*line = *(lepPacketP + 1);
		...
		if (*line == 20) {
			*seg = (*lepPacketP >> 4);
			if (debug_count < 10) {
				ESP_LOGI(TAG, "Segment packet: line=%d, seg=%d", *line, *seg);
			}
		} else if (debug_count < 10) {
			ESP_LOGI(TAG, "Data packet: line=%d", *line);
		}
