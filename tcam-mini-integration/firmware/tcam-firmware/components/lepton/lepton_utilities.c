...
bool lepton_init()
{
	...
	json_config_t* lep_stP = system_get_lep_st();
  	...
  	rsp = cci_run_ping();
  	if (rsp != 0) {
  		ESP_LOGE(TAG, "Lepton communication failed (%"PRIu32")", rsp);
  		return false;
	}
	...
	if (lep_is_radiometric) {
		...
		ESP_LOGI(TAG, "Lepton Radiometry = %"PRIu32"", rsp);
		...
		ESP_LOGI(TAG, "Lepton Radiometry TLinear = %"PRIu32"", rsp);
		if (rsp != val) {
			ESP_LOGE(TAG, "Lepton communication failed (%"PRIu32")", rsp);
	  		return false;
		}
		...
		ESP_LOGI(TAG, "Lepton Radiometry Auto Resolution = %"PRIu32"", rsp);
	...
	ESP_LOGI(TAG, "Lepton AGC Calcs = %"PRIu32"", rsp);
	...
	ESP_LOGI(TAG, "Lepton AGC = %"PRIu32"", rsp);
	if (rsp != val) {
		ESP_LOGE(TAG, "Lepton communication failed (%"PRIu32")", rsp);
  		return false;
	}
	...
	ESP_LOGI(TAG, "Lepton Telemetry = %"PRIu32"", rsp);
	...
	ESP_LOGI(TAG, "Lepton Gain Mode = %"PRIu32"", rsp);
	if (rsp != val) {
		ESP_LOGE(TAG, "Lepton communication failed (%"PRIu32")", rsp);
  		return false;
	}
	...
	ESP_LOGI(TAG, "Lepton GPIO Mode = %"PRIu32"", rsp);
