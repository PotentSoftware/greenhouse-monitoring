...
bool upd_init(uint32_t len, char* version)
{
	...
	if (update_partition == NULL) {
		...
		ESP_LOGI(TAG, "Update partion subtype %d at offset 0x%"PRIx32"", update_partition->subtype, update_partition->address);
	}
...
bool upd_process_bytes(uint32_t start, uint32_t len, uint8_t* buf)
{
	...
	if (start != cur_len) {
		ESP_LOGE(TAG, "upd_process_bytes exp %"PRIu32" does not match start %"PRIu32"", cur_len, start);
		return false;
	}
	...
	if (start == 0) {
		...
		if (len > sizeof(esp_image_header_t) + sizeof(esp_image_segment_header_t) + sizeof(esp_app_desc_t)) {
			...
			if (new_app_info.magic_word != ESP_APP_DESC_MAGIC_WORD) {
				ESP_LOGE(TAG, "FW Binary does not appear to be legal ESP32 binary, got %"PRIx32" instead of %x",
	...
	if (success) {
		...
		if (err != ESP_OK) {
			ESP_LOGE(TAG, "esp_ota_write failed at %"PRIu32" for %"PRIu32" bytes (%s)", start, len, esp_err_to_name(err));
			success = false;
		}
	}
